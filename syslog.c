/* Implementation of syslog via UDP for ESP8288_RTOS_SDK
 */
...
/* The following __xx macros are set in the component.mk makefile
 * based on the corresponding CONFIG_xx variables.
 */
#ifdef __EVENTGROUP
extern EventGroupHandle_t __EVENTGROUP;
#else
#error __EVENTGROUP not defined
#endif
#ifdef __NTP_SYNCED
extern const int __NTP_SYNCED;
#else
#error __NTP_SYNCED not defined
#endif
#ifdef __WIFI_CONNECTED
extern const int __WIFI_CONNECTED;
#else
#error __WIFI_CONNECTED not defined
#endif
#ifdef __MSG_QUEUED_VALUE
static const int MSG_QUEUED = __MSG_QUEUED_VALUE;
#else
#error __MSG_QUEUED_VALUE not defined
#endif
...
static void __send_udp()
{
    if (syslogQueue == NULL) {
        ...
        xEventGroupClearBits(__EVENTGROUP, MSG_QUEUED);
    }
    else {
        ESP_LOGD(TAG, "%s", __FUNCTION__);
...
static void syslog_task(void* pvParameter)
{
    ...
    while (1) {
        EventBits_t bits = xEventGroupWaitBits(__EVENTGROUP, __WIFI_CONNECTED, pdFALSE, pdFALSE, 100 / portTICK_PERIOD_MS);
        if (bits & __WIFI_CONNECTED) {
            switch (syslogState) {
                ...
                case SYSLOG_READY:
                    bits = xEventGroupWaitBits(__EVENTGROUP, MSG_QUEUED, pdFALSE, pdFALSE, 5000 / portTICK_PERIOD_MS);
                    if (bits & MSG_QUEUED) {
                        __send_udp();
                    }
                    break;
...
static void __add_entry(syslog_entry_t* entry)
{
    ...
    xEventGroupSetBits(__EVENTGROUP, MSG_QUEUED);
}
...
static syslog_entry_t* __compose_common(int facility, int severity, const char* app, char* ret, int msglen)
{
    ...
    if (__NTP_SYNCED & xEventGroupWaitBits(__EVENTGROUP, __NTP_SYNCED, pdFALSE, pdFALSE, 0)) {
